...
export function useDashboardPageLogic() {
  ...
  const normalizedExams = useMemo(
    () =>
      exams.map((exam) => ({
        ...exam,
        patientName:
          exam.patient?.name || exam.patient_name || exam.name || "Tanpa Nama",
        institution: exam.patient?.institution || exam.institution || "-",
  ...
  const recentExams = useMemo(() => filteredExams.slice(0, 4), [filteredExams]);
  ...
  const stats = useMemo(
    () => ({
      ...
      pending: normalizedExams.filter((e) => e.status === "Pending").length,
      done: normalizedExams.filter((e) => e.status === "Completed").length,
    }),
